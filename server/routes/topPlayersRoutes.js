const express = require('express');
const axios = require('axios');
const router = express.Router();

const token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiIsImtpZCI6IjI4YTMxOGY3LTAwMDAtYTFlYi03ZmExLTJjNzQzM2M2Y2NhNSJ9..7L1mFY2gUwt19jPDO00nSkFgddGg-1SwYcB8B3VsFVlKhBWKDL3meJyh779Itzz3fX3j6_GIVEJHEUsQM9oJ5g';

router.get('/list', async (req, res) => {
  try {
    const response = await axios.get('https://api.clashroyale.com/v1/leaderboards', {
      headers: { Authorization: `Bearer ${token}` }
    });
    res.json(response.data.items); // returns leaderboard list
  } 
  catch (err) {
    console.error('Leaderboard list error:', err.response?.data || err.message);
    res.status(500).json({ error: 'Failed to fetch leaderboard list' });
  }
});

module.exports = router;